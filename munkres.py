""" Variables:
		n      - number of vertices on both partitions
		U, V   - vertex sets
		lu, lv - are the labels of U and V, respectively

		the matching is encoded as 
			- a mapping Mu from U to V, 
			- a mapping Mv from V to U.
...
class Munkres:
	def __init__(self, lnodes, lnodes_labels, cnodes, cnodes_labels=None):
		self.lnodes = lnodes
		self.lnodes_labels = lnodes_labels
		self.cnodes = cnodes

		if cnodes_labels is None:
			self.cnodes_labels = [0 for _ in cnodes]
